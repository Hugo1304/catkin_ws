...
const double linear_max_speed = 1; // Vitesse maximale linéaire
const double angular_max_speed = 0.5; // Vitesse maximale angulaire
const double linear_acceleration = 0.1; // Accélération linéaire
const double angular_acceleration = 0.4; // Accélération angulaire
const double speed_increment = 0.2; // Incrément de vitesse
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        if (clavier == 'z') {
            ...
            linear_speed = 0.2;
            angular_speed = 0.0; // Réinitialiser la vitesse angulaire
        }
        // Tourner à gauche
        else if (clavier == 'q') {
        ...
        // Afficher la vitesse linéaire en temps réel
        ROS_INFO("Vitesse lineaire : %f", linear_speed);
    }
